...
// Per-backend execution context (bound on the OS thread for each Execute).
struct MetalExecCtx {
    id<MTLCommandQueue> queue;
    id<MTLCommandBuffer> encode_cb;
};

static pthread_key_t g_metal_exec_tls_key;
static int g_metal_exec_tls_created = 0;

static void metal_ensure_tls_key(void) {
    if (!g_metal_exec_tls_created) {
        pthread_key_create(&g_metal_exec_tls_key, NULL);
        g_metal_exec_tls_created = 1;
    }
}

static MetalExecCtx* metal_tls_get(void) {
    if (!g_metal_exec_tls_created) return NULL;
    return (MetalExecCtx*)pthread_getspecific(g_metal_exec_tls_key);
}

static id<MTLCommandQueue> metal_active_queue(void) {
    MetalExecCtx* c = metal_tls_get();
    if (c && c->queue) return c->queue;
    return g_commandQueue;
}

static id<MTLCommandBuffer> metal_active_batch_cb(void) {
    MetalExecCtx* c = metal_tls_get();
    return c ? c->encode_cb : nil;
}

static void metal_set_batch_cb(id<MTLCommandBuffer> cb) {
    MetalExecCtx* c = metal_tls_get();
    if (c) c->encode_cb = cb;
}

static id<MTLCommandBuffer> metal_dispatch_command_buffer(void) {
    id<MTLCommandBuffer> b = metal_active_batch_cb();
    if (b != nil) return b;
    return [metal_active_queue() commandBuffer];
}

MetalExecCtx* metal_exec_ctx_create(void) {
    metal_ensure_tls_key();
    if (!g_device) return NULL;
    MetalExecCtx* ctx = (MetalExecCtx*)calloc(1, sizeof(MetalExecCtx));
    if (!ctx) return NULL;
    ctx->queue = [g_device newCommandQueue];
    if (!ctx->queue) {
        free(ctx);
        return NULL;
    }
...
void metal_exec_ctx_destroy(MetalExecCtx* ctx) {
    if (!ctx) return;
    @autoreleasepool {
        if (ctx->encode_cb) {
            [ctx->encode_cb commit];
            [ctx->encode_cb waitUntilCompleted];
            ctx->encode_cb = nil;
        }
        ctx->queue = nil;
        free(ctx);
...
int metal_exec_thread_bind_ctx(MetalExecCtx* ctx) {
    metal_ensure_tls_key();
    if (!ctx) return -1;
    pthread_setspecific(g_metal_exec_tls_key, ctx);
    return 0;
}

void metal_exec_thread_unbind_ctx(void) {
    if (!g_metal_exec_tls_created) return;
    pthread_setspecific(g_metal_exec_tls_key, NULL);
}
...
void metal_encode_begin(void) {
    id<MTLCommandQueue> q = metal_active_queue();
    if (!q) return;
    if (metal_active_batch_cb() != nil) {
        NSLog(@"gomlx-metal: nested metal_encode_begin (ignored)");
        return;
    }
    metal_set_batch_cb([q commandBuffer]);
}

int metal_encode_barrier_wait(void) {
    id<MTLCommandBuffer> cb = metal_active_batch_cb();
    if (!cb) return 0;
    int r = commit_and_wait(cb);
    if (r != 0) {
        metal_set_batch_cb(nil);
        return r;
    }
    metal_set_batch_cb([metal_active_queue() commandBuffer]);
    return 0;
}
...
int metal_unary_op(const char* op_name, MetalBuffer src, MetalBuffer dst,
    ...
    @autoreleasepool {
        ...
        id<MTLCommandBuffer> cb = metal_dispatch_command_buffer();
        ...
        [enc endEncoding];
        if (metal_active_batch_cb() != nil && cb == metal_active_batch_cb()) return 0;
...
int metal_fused_dense(MetalBuffer a, MetalBuffer b, MetalBuffer bias, MetalBuffer c,
                      uint32_t m, uint32_t k, uint32_t n,
                      uint32_t has_bias, uint32_t activation, uint32_t gelu_exact,
                      int dtype) {
    @autoreleasepool {
        NSString* kn = [NSString stringWithFormat:@"fused_dense%@", dtype_suffix(dtype)];
        id<MTLComputePipelineState> pipeline = get_pipeline(kn);
        if (!pipeline) return -1;

        id<MTLBuffer> biasBuf = (__bridge id<MTLBuffer>)bias;
        if (!has_bias || bias == NULL) {
            biasBuf = g_dummyParamBuffer;
        }
        ...
        [enc setBuffer:(__bridge id<MTLBuffer>)a offset:0 atIndex:0];
        [enc setBuffer:(__bridge id<MTLBuffer>)b offset:0 atIndex:1];
        [enc setBuffer:biasBuf offset:0 atIndex:2];
        ...
        [enc setBytes:&activation length:sizeof(activation) atIndex:8];
        [enc setBytes:&gelu_exact length:sizeof(gelu_exact) atIndex:9];

        [enc setComputePipelineState:pipeline];
        uint32_t tile = 16;
        MTLSize gridSize = MTLSizeMake(((n + tile - 1) / tile) * tile, ((m + tile - 1) / tile) * tile, 1);
        MTLSize tgSize = MTLSizeMake(tile, tile, 1);
        [enc dispatchThreads:gridSize threadsPerThreadgroup:tgSize];
        [enc endEncoding];
        if (metal_active_batch_cb() != nil && cb == metal_active_batch_cb()) return 0;
...
int metal_fused_qkv_projection(
    MetalBuffer x, MetalBuffer w,
    MetalBuffer bias_q, MetalBuffer bias_k, MetalBuffer bias_v,
    MetalBuffer q_out, MetalBuffer k_out, MetalBuffer v_out,
    uint32_t m, uint32_t k, uint32_t q_dim, uint32_t kv_dim,
    uint32_t has_bias_q, uint32_t has_bias_k, uint32_t has_bias_v,
    int dtype) {
    @autoreleasepool {
        NSString* kn = [NSString stringWithFormat:@"fused_qkv_projection%@", dtype_suffix(dtype)];
        id<MTLComputePipelineState> pipeline = get_pipeline(kn);
        if (!pipeline) return -1;

        id<MTLBuffer> bq = (__bridge id<MTLBuffer>)bias_q;
        id<MTLBuffer> bk = (__bridge id<MTLBuffer>)bias_k;
        id<MTLBuffer> bv = (__bridge id<MTLBuffer>)bias_v;
        if (!has_bias_q || bias_q == NULL) bq = g_dummyParamBuffer;
        if (!has_bias_k || bias_k == NULL) bk = g_dummyParamBuffer;
        if (!has_bias_v || bias_v == NULL) bv = g_dummyParamBuffer;

        uint32_t ntot = q_dim + 2 * kv_dim;
        id<MTLCommandBuffer> cb = metal_dispatch_command_buffer();
        id<MTLComputeCommandEncoder> enc = [cb computeCommandEncoder];
        ...
        [enc setBuffer:(__bridge id<MTLBuffer>)q_out offset:0 atIndex:5];
        [enc setBuffer:(__bridge id<MTLBuffer>)k_out offset:0 atIndex:6];
        [enc setBuffer:(__bridge id<MTLBuffer>)v_out offset:0 atIndex:7];
        [enc setBytes:&m length:sizeof(m) atIndex:8];
        [enc setBytes:&k length:sizeof(k) atIndex:9];
        [enc setBytes:&q_dim length:sizeof(q_dim) atIndex:10];
        [enc setBytes:&kv_dim length:sizeof(kv_dim) atIndex:11];
        [enc setBytes:&has_bias_q length:sizeof(has_bias_q) atIndex:12];
        [enc setBytes:&has_bias_k length:sizeof(has_bias_k) atIndex:13];
        [enc setBytes:&has_bias_v length:sizeof(has_bias_v) atIndex:14];

        [enc setComputePipelineState:pipeline];
        uint32_t tile = 16;
        MTLSize gridSize = MTLSizeMake(((ntot + tile - 1) / tile) * tile, ((m + tile - 1) / tile) * tile, 1);
        MTLSize tgSize = MTLSizeMake(tile, tile, 1);
        [enc dispatchThreads:gridSize threadsPerThreadgroup:tgSize];
        [enc endEncoding];
        if (metal_active_batch_cb() != nil && cb == metal_active_batch_cb()) return 0;
...
static NSString* convert_kernel_name(int src_dtype, int dst_dtype) {
    ...
    if (src_dtype >= 0 && src_dtype <= 12 &&
        dst_dtype >= 0 && dst_dtype <= 12 &&
...
int metal_dynamic_slice(MetalBuffer src, MetalBuffer dst,
                        MetalBuffer config, uint32_t total_elements,
                        uint32_t config_size, uint32_t elem_size) {
    @autoreleasepool {
        id<MTLComputePipelineState> pipeline = get_pipeline(@"dynamic_slice_bytes");
        if (!pipeline) return -1;
...
// MaxPool2D gradient NCHW: dtype 1=float32, 0=float16
int metal_max_pool2d_grad_nchw(MetalBuffer operand, MetalBuffer dy, MetalBuffer dx,
                               MetalBuffer config, uint32_t total_out, int dtype) {
    (void)dtype;
    @autoreleasepool {
        id<MTLComputePipelineState> pipeline = get_pipeline(@"max_pool2d_grad_nchw_f32");
        if (!pipeline) return -1;
        ...
        [enc setBuffer:(__bridge id<MTLBuffer>)dy      offset:0 atIndex:1];
        [enc setBuffer:(__bridge id<MTLBuffer>)dx      offset:0 atIndex:2];
        [enc setBuffer:(__bridge id<MTLBuffer>)config offset:0 atIndex:3];
        [enc setBytes:&total_out length:sizeof(uint32_t) atIndex:4];

        dispatch_1d(pipeline, enc, total_out);
        [enc endEncoding];
        if (metal_active_batch_cb() != nil && cb == metal_active_batch_cb()) return 0;
        return commit_and_wait(cb);
    }
...
int metal_dynamic_update_slice(MetalBuffer operand, MetalBuffer update, MetalBuffer dst,
                               MetalBuffer config, uint32_t total_elements,
                               uint32_t config_size, uint32_t elem_size) {
    @autoreleasepool {
        id<MTLComputePipelineState> pipeline = get_pipeline(@"dynamic_update_slice_bytes");
        if (!pipeline) return -1;
        ...
        [enc setBuffer:(__bridge id<MTLBuffer>)operand offset:0 atIndex:0];
        [enc setBuffer:(__bridge id<MTLBuffer>)update offset:0 atIndex:1];
        [enc setBuffer:(__bridge id<MTLBuffer>)dst     offset:0 atIndex:2];
        [enc setBuffer:(__bridge id<MTLBuffer>)config offset:0 atIndex:3];
...
int metal_sort_load_pair_direct_bytes(MetalBuffer flat, MetalBuffer lhs, MetalBuffer rhs,
                                      uint32_t base_elem, uint32_t axis_stride, uint32_t elem_size,
                                      uint32_t ii, uint32_t jj) {
    @autoreleasepool {
        id<MTLComputePipelineState> pipeline = get_pipeline(@"sort_load_pair_direct_bytes");
        if (!pipeline) return -1;
        ...
        [enc setBytes:&ij length:sizeof(ij) atIndex:4];
...
static int metal_run_scatter_64_fast(
    ...
    if (metal_encode_barrier_wait() != 0) return -6;
