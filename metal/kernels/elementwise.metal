...
// Uint32 arithmetic / comparisons (parity with int32; bitwise and/or/xor below).
#define U32_BIN(NAME, EXPR)                                                       \
kernel void NAME##_u32(device const uint* lhs [[buffer(0)]],                    \
                 device const uint* rhs [[buffer(1)]],                            \
                 device uint* dst       [[buffer(2)]],                            \
                 uint id [[thread_position_in_grid]]) {                            \
    uint a = lhs[id], b = rhs[id];                                                \
    dst[id] = EXPR;                                                                \
}

U32_BIN(op_add_u32, a + b)
U32_BIN(op_sub_u32, a - b)
U32_BIN(op_mul_u32, a * b)
U32_BIN(op_div_u32, (b != 0u) ? (a / b) : 0u)
U32_BIN(op_rem_u32, (b != 0u) ? (a % b) : 0u)
U32_BIN(op_max_u32, max(a, b))
U32_BIN(op_min_u32, min(a, b))
U32_BIN(op_pow_u32, uint(int(pow(float(a), float(b)))))
U32_BIN(op_atan2_u32, uint(rint(atan2(float(a), float(b)))))

#define CMP_PRED_KERNEL_U32(NAME, OP)                                             \
kernel void NAME##_u32(device const uint* lhs [[buffer(0)]],                      \
                       device const uint* rhs [[buffer(1)]],                       \
                       device uchar* dst       [[buffer(2)]],                      \
                       uint id [[thread_position_in_grid]]) {                      \
...
CMP_PRED_KERNEL_U32(op_equal_pred, ==)
CMP_PRED_KERNEL_U32(op_not_equal_pred, !=)
CMP_PRED_KERNEL_U32(op_less_pred, <)
CMP_PRED_KERNEL_U32(op_less_or_equal_pred, <=)
CMP_PRED_KERNEL_U32(op_greater_pred, >)
CMP_PRED_KERNEL_U32(op_greater_or_equal_pred, >=)

kernel void op_neg_u32(device const uint* src [[buffer(0)]],
                       device uint* dst       [[buffer(1)]],
                       uint id [[thread_position_in_grid]]) {
    dst[id] = 0u - src[id];
}
kernel void op_abs_u32(device const uint* src [[buffer(0)]],
                       device uint* dst       [[buffer(1)]],
                       uint id [[thread_position_in_grid]]) {
    dst[id] = src[id];
}
kernel void op_sign_u32(device const uint* src [[buffer(0)]],
                        device uint* dst       [[buffer(1)]],
                        uint id [[thread_position_in_grid]]) {
    uint x = src[id];
    dst[id] = (x > 0u) ? 1u : 0u;
...
#define I64_BIN(NAME, EXPR)                                                       \
kernel void NAME##_i64(device const long* lhs [[buffer(0)]],                     \
                 device const long* rhs [[buffer(1)]],                            \
                 device long* dst       [[buffer(2)]],                            \
                 uint id [[thread_position_in_grid]]) {                           \
    long a = lhs[id], b = rhs[id];                                                \
    dst[id] = EXPR;                                                                \
}

I64_BIN(op_add_i64, a + b)
I64_BIN(op_sub_i64, a - b)
I64_BIN(op_mul_i64, a * b)
I64_BIN(op_div_i64, (b != 0l) ? (a / b) : 0l)
I64_BIN(op_rem_i64, (b != 0l) ? (a % b) : 0l)
I64_BIN(op_max_i64, max(a, b))
I64_BIN(op_min_i64, min(a, b))
I64_BIN(op_pow_i64, long(pow(float(a), float(b))))
I64_BIN(op_atan2_i64, long(rint(atan2(float(a), float(b)))))
I64_BIN(op_bitwise_and_i64, a & b)
I64_BIN(op_bitwise_or_i64, a | b)
I64_BIN(op_bitwise_xor_i64, a ^ b)

#define CMP_PRED_KERNEL_I64(NAME, OP)                                             \
kernel void NAME##_i64(device const long* lhs [[buffer(0)]],                     \
                       device const long* rhs [[buffer(1)]],                       \
                       device uchar* dst       [[buffer(2)]],                      \
                       uint id [[thread_position_in_grid]]) {                      \
...
CMP_PRED_KERNEL_I64(op_equal_pred, ==)
CMP_PRED_KERNEL_I64(op_not_equal_pred, !=)
CMP_PRED_KERNEL_I64(op_less_pred, <)
CMP_PRED_KERNEL_I64(op_less_or_equal_pred, <=)
CMP_PRED_KERNEL_I64(op_greater_pred, >)
CMP_PRED_KERNEL_I64(op_greater_or_equal_pred, >=)

kernel void op_neg_i64(device const long* src [[buffer(0)]],
                       device long* dst       [[buffer(1)]],
                       uint id [[thread_position_in_grid]]) {
    dst[id] = -src[id];
}
kernel void op_abs_i64(device const long* src [[buffer(0)]],
                       device long* dst       [[buffer(1)]],
                       uint id [[thread_position_in_grid]]) {
    long x = src[id];
    dst[id] = x < 0 ? -x : x;
}
kernel void op_sign_i64(device const long* src [[buffer(0)]],
                        device long* dst       [[buffer(1)]],
                        uint id [[thread_position_in_grid]]) {
    long x = src[id];
    dst[id] = (x > 0) - (x < 0);
}
kernel void op_bitwise_not_i64(device const long* src [[buffer(0)]],
                               device long* dst       [[buffer(1)]],
                               uint id [[thread_position_in_grid]]) {
    dst[id] = ~src[id];
}
kernel void op_clz_i64(device const long* src [[buffer(0)]],
                       device long* dst       [[buffer(1)]],
                       uint id [[thread_position_in_grid]]) {
    dst[id] = long(clz(as_type<ulong>(src[id])));
}
kernel void op_bitcount_i64(device const long* src [[buffer(0)]],
                            device long* dst       [[buffer(1)]],
                            uint id [[thread_position_in_grid]]) {
    dst[id] = long(popcount(as_type<ulong>(src[id])));
}

// Uint64 elementwise (device ulong).
#define U64_BIN(NAME, EXPR)                                                       \
kernel void NAME##_u64(device const ulong* lhs [[buffer(0)]],                    \
                 device const ulong* rhs [[buffer(1)]],                           \
                 device ulong* dst       [[buffer(2)]],                           \
                 uint id [[thread_position_in_grid]]) {                           \
    ulong a = lhs[id], b = rhs[id];                                               \
    dst[id] = EXPR;                                                                \
}

U64_BIN(op_add_u64, a + b)
U64_BIN(op_sub_u64, a - b)
U64_BIN(op_mul_u64, a * b)
U64_BIN(op_div_u64, (b != 0ul) ? (a / b) : 0ul)
U64_BIN(op_rem_u64, (b != 0ul) ? (a % b) : 0ul)
U64_BIN(op_max_u64, max(a, b))
U64_BIN(op_min_u64, min(a, b))
U64_BIN(op_pow_u64, ulong(pow(float(a), float(b))))
U64_BIN(op_atan2_u64, ulong(rint(atan2(float(a), float(b)))))
U64_BIN(op_bitwise_and_u64, a & b)
U64_BIN(op_bitwise_or_u64, a | b)
U64_BIN(op_bitwise_xor_u64, a ^ b)

#define CMP_PRED_KERNEL_U64(NAME, OP)                                            \
kernel void NAME##_u64(device const ulong* lhs [[buffer(0)]],                    \
                       device const ulong* rhs [[buffer(1)]],                      \
                       device uchar* dst       [[buffer(2)]],                      \
                       uint id [[thread_position_in_grid]]) {                      \
...
CMP_PRED_KERNEL_U64(op_equal_pred, ==)
CMP_PRED_KERNEL_U64(op_not_equal_pred, !=)
CMP_PRED_KERNEL_U64(op_less_pred, <)
CMP_PRED_KERNEL_U64(op_less_or_equal_pred, <=)
CMP_PRED_KERNEL_U64(op_greater_pred, >)
CMP_PRED_KERNEL_U64(op_greater_or_equal_pred, >=)

kernel void op_neg_u64(device const ulong* src [[buffer(0)]],
                       device ulong* dst       [[buffer(1)]],
                       uint id [[thread_position_in_grid]]) {
    dst[id] = 0ul - src[id];
}
kernel void op_abs_u64(device const ulong* src [[buffer(0)]],
                       device ulong* dst       [[buffer(1)]],
                       uint id [[thread_position_in_grid]]) {
    dst[id] = src[id];
}
kernel void op_sign_u64(device const ulong* src [[buffer(0)]],
                        device ulong* dst       [[buffer(1)]],
                        uint id [[thread_position_in_grid]]) {
    ulong x = src[id];
    dst[id] = (x > 0ul) ? 1ul : 0ul;
}
kernel void op_bitwise_not_u64(device const ulong* src [[buffer(0)]],
                               device ulong* dst       [[buffer(1)]],
                               uint id [[thread_position_in_grid]]) {
    dst[id] = ~src[id];
}
kernel void op_clz_u64(device const ulong* src [[buffer(0)]],
                       device ulong* dst       [[buffer(1)]],
                       uint id [[thread_position_in_grid]]) {
    dst[id] = ulong(clz(src[id]));
}
kernel void op_bitcount_u64(device const ulong* src [[buffer(0)]],
                            device ulong* dst       [[buffer(1)]],
                            uint id [[thread_position_in_grid]]) {
    dst[id] = ulong(popcount(src[id]));
}
...
// Shifts: shift amount follows XLA/HLO (low bits of rhs, modulo element bit width).
kernel void op_shift_left_i32(device const int* lhs [[buffer(0)]],
                              device const int* rhs [[buffer(1)]],
                              device int* dst       [[buffer(2)]],
                              uint id [[thread_position_in_grid]]) {
    uint sh = uint(rhs[id]) & 31u;
    dst[id] = as_type<int>(as_type<uint>(lhs[id]) << sh);
}
kernel void op_shift_left_u32(device const uint* lhs [[buffer(0)]],
                              device const uint* rhs [[buffer(1)]],
                              device uint* dst       [[buffer(2)]],
                              uint id [[thread_position_in_grid]]) {
    uint sh = uint(rhs[id]) & 31u;
...
kernel void op_shift_right_arith_i32(device const int* lhs [[buffer(0)]],
                                   device const int* rhs [[buffer(1)]],
                                   device int* dst       [[buffer(2)]],
                                   uint id [[thread_position_in_grid]]) {
    uint sh = uint(rhs[id]) & 31u;
    dst[id] = lhs[id] >> int(sh);
}
kernel void op_shift_right_logical_u32(device const uint* lhs [[buffer(0)]],
                                       device const uint* rhs [[buffer(1)]],
                                       device uint* dst       [[buffer(2)]],
                                       uint id [[thread_position_in_grid]]) {
    uint sh = uint(rhs[id]) & 31u;
...
kernel void op_shift_right_logical_i32(device const int* lhs [[buffer(0)]],
                                       device const int* rhs [[buffer(1)]],
                                       device int* dst       [[buffer(2)]],
                                       uint id [[thread_position_in_grid]]) {
    uint sh = uint(rhs[id]) & 31u;
...
// 64-bit shifts (native width; rhs uses low bits modulo 64, same rule as 32-bit kernels).
kernel void op_shift_left_i64(device const long* lhs [[buffer(0)]],
                              device const long* rhs [[buffer(1)]],
                              device long* dst       [[buffer(2)]],
                              uint id [[thread_position_in_grid]]) {
    uint sh = uint(as_type<ulong>(rhs[id]) & 63ul);
    dst[id] = as_type<long>(as_type<ulong>(lhs[id]) << sh);
}
kernel void op_shift_left_u64(device const ulong* lhs [[buffer(0)]],
                              device const ulong* rhs [[buffer(1)]],
                              device ulong* dst       [[buffer(2)]],
                              uint id [[thread_position_in_grid]]) {
    uint sh = uint(rhs[id] & 63ul);
    dst[id] = lhs[id] << sh;
}
kernel void op_shift_right_arith_i64(device const long* lhs [[buffer(0)]],
                                     device const long* rhs [[buffer(1)]],
                                     device long* dst       [[buffer(2)]],
                                     uint id [[thread_position_in_grid]]) {
    uint sh = uint(as_type<ulong>(rhs[id]) & 63ul);
    dst[id] = lhs[id] >> long(sh);
}
kernel void op_shift_right_logical_u64(device const ulong* lhs [[buffer(0)]],
                                       device const ulong* rhs [[buffer(1)]],
                                       device ulong* dst       [[buffer(2)]],
                                       uint id [[thread_position_in_grid]]) {
    uint sh = uint(rhs[id] & 63ul);
...
kernel void op_shift_right_logical_i64(device const long* lhs [[buffer(0)]],
                                       device const long* rhs [[buffer(1)]],
                                       device long* dst       [[buffer(2)]],
                                       uint id [[thread_position_in_grid]]) {
    uint sh = uint(as_type<ulong>(rhs[id]) & 63ul);
