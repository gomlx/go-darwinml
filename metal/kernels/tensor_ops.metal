...
// DynamicSlice: same indexing as slice_bytes; start offsets in config are clamped on the host
// (XLA: start_i = clamp(start_i, 0, operand_dim_i - slice_size_i)).
kernel void dynamic_slice_bytes(
    device const uchar* src       [[buffer(0)]],
    device uchar* dst             [[buffer(1)]],
    device const uint* config     [[buffer(2)]],
    constant uint& total_elements [[buffer(3)]],
...
// MaxPool2D gradient (SelectAndScatterMax), operand layout NCHW — matches reduce_window
// coordinate mapping (pad_low per axis, valid cells only). One thread per output element;
// scatters dy[id] into dx[argmax_flat] with tie-break lower linear window index.
// Only float32 accumulators are implemented here; float16 uses this kernel after promoting
// operands in the Go executor (portable half atomics are not assumed).
kernel void max_pool2d_grad_nchw_f32(
    device const float* operand [[buffer(0)]],
    device const float* dy      [[buffer(1)]],
    device float* dx            [[buffer(2)]],
    device const uint* config   [[buffer(3)]],
    constant uint& total_out    [[buffer(4)]],
    uint id [[thread_position_in_grid]])
{
    ...
    for (uint wi = 0; wi < win_total; wi++) {
        ...
        float val = operand[in_flat];
        if (!found || val > best_val || (val == best_val && wi < best_wi)) {
            found = true;
            best_val = val;
            best_in_flat = in_flat;
            best_wi = wi;
        }
    }

    if (found) {
        device atomic_float* addr = (device atomic_float*)(dx + best_in_flat);
        atomic_fetch_add_explicit(addr, dy[id], memory_order_relaxed);
    }
}

// DynamicUpdateSlice: output same shape as operand; writes update at clamped starts.
// Buffers: operand (0), update (1), dst (2), config (3).
// Config layout:
//   [0] = rank
//   [1 .. rank]              = starts[rank] (clamped, host-packed)
//   [rank+1 .. 2*rank]       = operand_dims[rank]
//   [2*rank+1 .. 3*rank]     = update_dims[rank]
//   [3*rank+1 .. 4*rank]     = operand_strides (row-major)
//   [4*rank+1 .. 5*rank]     = update_strides (row-major)
kernel void dynamic_update_slice_bytes(
    device const uchar* operand     [[buffer(0)]],
    device const uchar* update      [[buffer(1)]],
    ...
    uint id [[thread_position_in_grid]])
{
    ...
    device const uint* starts          = config + 1;
    device const uint* operand_dims    = config + 1 + rank;
    device const uint* update_dims     = config + 1 + 2 * rank;
    device const uint* operand_strides = config + 1 + 3 * rank;
    device const uint* update_strides  = config + 1 + 4 * rank;

    uint remaining = id;
    uint out_coord[16];
    for (uint d = 0; d < rank; d++) {
        uint out_stride = 1;
        for (uint dd = d + 1; dd < rank; dd++) out_stride *= operand_dims[dd];
        uint coord = remaining / out_stride;
        remaining %= out_stride;
        out_coord[d] = coord;
    }
    ...
    for (uint d = 0; d < rank; d++) {
        uint lo = starts[d];
        uint hi = starts[d] + update_dims[d];
        if (out_coord[d] < lo || out_coord[d] >= hi) {
            inside = false;
            break;
        }
    }
    ...
    if (inside) {
        for (uint d = 0; d < rank; d++) {
            src_idx += (out_coord[d] - starts[d]) * update_strides[d];
        }
        uint src_off = src_idx * elem_size;
        for (uint b = 0; b < elem_size; b++) {
            dst[dst_off + b] = update[src_off + b];
        }
    } else {
        for (uint d = 0; d < rank; d++) {
            src_idx += out_coord[d] * operand_strides[d];
        }
        uint src_off = src_idx * elem_size;
        for (uint b = 0; b < elem_size; b++) {
            dst[dst_off + b] = operand[src_off + b];
        }
    }
}
...
// BFloat16 storage is uint16 top bits of float32 (HLO / XLA semantics).
static inline float mosaic_bf16_bits_to_f32(ushort bits) {
    uint u = uint(bits) << 16;
    return as_type<float>(u);
}
static inline ushort mosaic_f32_to_bf16_bits(float x) {
...
static inline uint convert_kind_size(uint kind) {
    switch (kind) {
        ...
        case 12: return sizeof(ushort); // BFloat16
        default: return 0;
    }
}
...
static inline bool convert_load_bool(device const uchar* src, uint off, uint kind) {
    switch (kind) {
        ...
        case 12: return mosaic_bf16_bits_to_f32(LOAD_SCALAR(ushort, src, off)) != 0.0f;
...
static inline long convert_load_i64(device const uchar* src, uint off, uint kind) {
    switch (kind) {
        ...
        case 12: return long(mosaic_bf16_bits_to_f32(LOAD_SCALAR(ushort, src, off)));
        default: return 0l;
    }
}
...
static inline ulong convert_load_u64(device const uchar* src, uint off, uint kind) {
    switch (kind) {
        ...
        case 12: return ulong(mosaic_bf16_bits_to_f32(LOAD_SCALAR(ushort, src, off)));
        default: return 0ul;
    }
}
...
static inline float convert_load_f32(device const uchar* src, uint off, uint kind) {
    switch (kind) {
        ...
        case 12: return mosaic_bf16_bits_to_f32(LOAD_SCALAR(ushort, src, off));
        default: return 0.0f;
    }
}
...
kernel void convert_dtype(device const uchar* src [[buffer(0)]],
    ...
    switch (dst_kind) {
        ...
        case 11:
            ...
            break;
        case 12:
            STORE_SCALAR(ushort, dst, dst_off,
                         mosaic_f32_to_bf16_bits(convert_load_f32(src, src_off, src_kind)));
            break;
        default:
            break;
    }
...
// Comparator prep for Sort merge: copy elements at original axis indices ii / jj.
kernel void sort_load_pair_direct_bytes(device const uchar* flat [[buffer(0)]],
                                        device uchar* lhs [[buffer(1)]],
                                        device uchar* rhs [[buffer(2)]],
                                        constant uint4& base_stride_es [[buffer(3)]],
                                        constant uint2& ij [[buffer(4)]],
                                        uint tid [[thread_position_in_grid]]) {
    if (tid != 0) return;
    uint base = base_stride_es.x;
    uint axis_stride = base_stride_es.y;
    uint es = base_stride_es.z;
    uint ii = ij.x;
    uint jj = ij.y;
    uint e0 = (base + ii * axis_stride) * es;
    uint e1 = (base + jj * axis_stride) * es;
    for (uint b = 0; b < es; b++) {
        lhs[b] = flat[e0 + b];
        rhs[b] = flat[e1 + b];
    }
