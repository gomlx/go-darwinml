...
type Backend struct {
	...
	// execCtx holds a dedicated MTLCommandQueue and batched encoder for this backend.
	execCtx *C.MetalExecCtx
}
...
func New(config string) (backends.Backend, error) {
	...
	ctx := C.metal_exec_ctx_create()
	if ctx == nil {
		metalBackendUsers.Add(-1)
		return nil, errors.New("metal: failed to create per-backend command queue")
	}
	return &Backend{execCtx: ctx}, nil
}
...
func (b *Backend) Finalize() {
	...
	b.isFinalized = true
	if b.execCtx != nil {
		C.metal_exec_ctx_destroy(b.execCtx)
		b.execCtx = nil
	}
...
// 6=_i64 and 7=_u64 are reserved for metal_binary_op / dtype_suffix (64-bit shift kernels only).
...
// cDynamicUpdateSlice wraps metal_dynamic_update_slice so executable.go need not
// reference this C symbol directly (same rationale as cTransposePerm for gopls/cgo).
func cDynamicUpdateSlice(operand, update, dest, config *Buffer, totalElements, configWords, elemSize int) error {
	if ret := C.metal_dynamic_update_slice(
		operand.mtl, update.mtl, dest.mtl, config.mtl,
		C.uint32_t(totalElements), C.uint32_t(configWords), C.uint32_t(elemSize)); ret != 0 {
		return errors.Errorf("metal_dynamic_update_slice failed: %d", ret)
	}

	return nil
}

func cDynamicSlice(src, dest, config *Buffer, totalElements, configWords, elemSize int) error {
	if ret := C.metal_dynamic_slice(
		src.mtl, dest.mtl, config.mtl,
		C.uint32_t(totalElements), C.uint32_t(configWords), C.uint32_t(elemSize)); ret != 0 {
		return errors.Errorf("metal_dynamic_slice failed: %d", ret)
	}
	return nil
}

func cMaxPool2DGradNCHW(operand, dy, dx, config *Buffer, totalOut int, dtype C.int) error {
	if ret := C.metal_max_pool2d_grad_nchw(
		operand.mtl, dy.mtl, dx.mtl, config.mtl,
		C.uint32_t(totalOut), dtype); ret != 0 {
		return errors.Errorf("metal_max_pool2d_grad_nchw failed: %d", ret)
	}
	return nil
}
