...
// Per-thread execution context: separate MTLCommandQueue + batched encoder so
// independent Backend values can run Execute concurrently (each binds its context
// with metal_exec_thread_bind_ctx for the duration of runtime.LockOSThread).
typedef struct MetalExecCtx MetalExecCtx;
...
// src_dtype/dst_dtype: 0=f16, 1=f32, 2=f64, 3=i32, 4=i64, 12=bfloat16
...
// Dynamic slice: same config layout as Slice; starts must be host-clamped.
int metal_dynamic_slice(MetalBuffer src, MetalBuffer dst,
                        MetalBuffer config, uint32_t total_elements,
                        uint32_t config_size, uint32_t elem_size);

// SelectAndScatterMax / MaxPool2DGradient: float32 kernel only; config matches reduce_window padding section.
// The dtype argument is reserved; float16 is promoted to float32 in Go before calling.
...
// DynamicUpdateSlice: operand + update + dst; config = [rank, starts, operand_dims,
// update_dims, operand_strides, update_strides] (each rank wide).
...
// Sort merge: load comparator lhs/rhs from flat at original axis indices ii, jj (element offsets).
