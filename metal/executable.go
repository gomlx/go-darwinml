...
// gpuSyncBeforeHostRead finishes the in-flight batched command buffer (if any) so
// StorageModeShared memory is safe to read or memcpy on the CPU.
func gpuSyncBeforeHostRead() error {
	return metalHostGPUSync()
}
...
func (e *Executable) Execute(inputs []backends.Buffer, donate []bool, defaultDevice backends.DeviceNum) (res []backends.Buffer, err error) {
	...
	rc := C.metal_exec_thread_bind_ctx(b.execCtx)
	if rc != 0 {
		return nil, errors.Errorf("metal: bind exec context failed (%d)", rc)
	}
	defer C.metal_exec_thread_unbind_ctx()
...
func executeNode(backend *Backend, node *Node, inputs []*Buffer) (*Buffer, error) {
	...
	case backends.OpTypeAdd, backends.OpTypeSub, backends.OpTypeMul, backends.OpTypeDiv,
		...
		backends.OpTypeShiftLeft, backends.OpTypeShiftRightArithmetic, backends.OpTypeShiftRightLogical,
	...
	case backends.OpTypeDynamicSlice:
		return executeDynamicSlice(node, inputs)
	case backends.OpTypeDynamicUpdateSlice:
		return executeDynamicUpdateSlice(node, inputs)
	...
	case backends.OpTypeSelectAndScatterMax:
		return executeSelectAndScatterMax(node, inputs)
	...
	case backends.OpTypeFusedDense:
		return executeFusedDense(node, inputs)
...
func executeUnary(node *Node, input *Buffer) (*Buffer, error) {
	...
	origElemDT := input.shape.DType
	in := input
	if input.shape.DType == dtypes.BFloat16 {
		b, err := gpuConvertSameShape(input, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		in = b
	}
	outIsBF16 := node.shape.DType == dtypes.BFloat16

	var dt C.int
	if backends.OpTypeLogicalNot == node.opType && input.shape.DType == dtypes.Bool {
		dt = metalDTypeBoolPred()
	} else if workDT, code, ok2 := elementwiseIntegerWorkDType(in.shape.DType); ok2 {
		if in.shape.DType != workDT {
			w, err := gpuConvertSameShape(in, workDT)
			if err != nil {
				return nil, err
			}
			in = w
		...
		dt = elementwiseDTypeToMetal(in.shape.DType)
		if dt < 0 {
			return nil, errors.Errorf("metal: unsupported unary input dtype %s", in.shape.DType)
		}
	}

	execShape := node.shape
	if outIsBF16 {
		execShape = shapes.Make(dtypes.Float32, node.shape.Dimensions...)
	} else if workDT, _, ok2 := elementwiseIntegerWorkDType(origElemDT); ok2 {
		execShape = shapes.Make(workDT, node.shape.Dimensions...)
	}

	dst := allocDuringExec(execShape)
	cName := C.CString(name)
	defer C.free(unsafe.Pointer(cName))
	n := C.uint32_t(node.shape.Size())

	if ret := C.metal_unary_op(cName, in.mtl, dst.mtl, n, dt); ret != 0 {
		return nil, errors.Errorf("metal_unary_op(%s) failed: %d", name, ret)
	}
	if outIsBF16 {
		return gpuConvertSameShape(dst, dtypes.BFloat16)
	}
	if workDT, _, ok2 := elementwiseIntegerWorkDType(origElemDT); ok2 && workDT != node.shape.DType {
		return gpuConvertSameShape(dst, node.shape.DType)
	}
	return dst, nil
}
...
// shiftBinaryMetalKernel returns the elementwise Metal kernel base name for shift ops.
// Unsigned ShiftRightArithmetic uses logical right shift (standard for unsigned integers).
func shiftBinaryMetalKernel(opType backends.OpType, dt dtypes.DType) (base string, err error) {
	switch opType {
	case backends.OpTypeShiftLeft:
		return "op_shift_left", nil
	case backends.OpTypeShiftRightArithmetic:
		switch dt {
		case dtypes.Uint8, dtypes.Uint16, dtypes.Uint32, dtypes.Uint64:
			return "op_shift_right_logical", nil
		default:
			return "op_shift_right_arith", nil
		}
	case backends.OpTypeShiftRightLogical:
		return "op_shift_right_logical", nil
	default:
		return "", errors.Errorf("metal: internal: %s is not a shift op", opType)
	}
}

func executeShiftBinary(node *Node, lhs, rhs *Buffer) (*Buffer, error) {
	if !lhs.shape.Equal(rhs.shape) {
		return nil, errors.Errorf("metal: shift op %s requires matching lhs/rhs shapes, got %s vs %s",
			node.opType, lhs.shape, rhs.shape)
	}
	if !lhs.shape.Equal(node.shape) {
		return nil, errors.Errorf("metal: shift op %s output shape %s vs operands %s",
			node.opType, node.shape, lhs.shape)
	}
	dt := lhs.shape.DType
	if rhs.shape.DType != dt {
		return nil, errors.Errorf("metal: shift operands dtype mismatch %s vs %s", dt, rhs.shape.DType)
	}

	/* Narrow integer dtypes promote to Int32/Uint32; Int64/Uint64 use native-width shift kernels. */
	var workDT dtypes.DType
	var metalDT C.int
	switch dt {
	case dtypes.Int8, dtypes.Int16, dtypes.Int32:
		workDT = dtypes.Int32
		metalDT = 5
	case dtypes.Int64:
		workDT = dtypes.Int64
		metalDT = 6
	case dtypes.Uint8, dtypes.Uint16, dtypes.Uint32:
		workDT = dtypes.Uint32
		metalDT = 3
	case dtypes.Uint64:
		workDT = dtypes.Uint64
		metalDT = 7
	default:
		return nil, errors.Errorf("metal: shift ops require integer dtypes, got %s", dt)
	}

	base, err := shiftBinaryMetalKernel(node.opType, dt)
	if err != nil {
		return nil, err
	}
	...
	l, r := lhs, rhs
	if dt != workDT {
		lhsW, err := gpuConvertSameShape(lhs, workDT)
		if err != nil {
			return nil, err
		}
		rhsW, err := gpuConvertSameShape(rhs, workDT)
		if err != nil {
			return nil, err
		}
		l, r = lhsW, rhsW
	...
	dstWork := allocDuringExec(shapes.Make(workDT, node.shape.Dimensions...))
	cName := C.CString(base)
	defer C.free(unsafe.Pointer(cName))
	n := C.uint32_t(node.shape.Size())

	if ret := C.metal_binary_op(cName, l.mtl, r.mtl, dstWork.mtl, n, metalDT); ret != 0 {
		return nil, errors.Errorf("metal_binary_op(%s) failed: %d", base, ret)
	}
	if dt == workDT {
		return dstWork, nil
	}
	return gpuConvertSameShape(dstWork, dt)
}

// elementwiseIntegerWorkDType maps narrow integer dtypes to the width used by elementwise .metal kernels.
func elementwiseIntegerWorkDType(dt dtypes.DType) (work dtypes.DType, code C.int, ok bool) {
	switch dt {
	case dtypes.Int8, dtypes.Int16, dtypes.Int32:
		return dtypes.Int32, 5, true
	case dtypes.Uint8, dtypes.Uint16, dtypes.Uint32:
		return dtypes.Uint32, 3, true
	case dtypes.Int64:
		return dtypes.Int64, 6, true
	case dtypes.Uint64:
		return dtypes.Uint64, 7, true
	default:
		return dtypes.InvalidDType, -1, false
	}
}

func executeBinary(node *Node, lhs, rhs *Buffer) (*Buffer, error) {
	switch node.opType {
	case backends.OpTypeShiftLeft, backends.OpTypeShiftRightArithmetic, backends.OpTypeShiftRightLogical:
		return executeShiftBinary(node, lhs, rhs)
	default:
	}
	...
	origElemDT := lhs.shape.DType
	l, r := lhs, rhs
	if lhs.shape.DType == dtypes.BFloat16 {
		b, err := gpuConvertSameShape(lhs, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		l = b
	}
	if rhs.shape.DType == dtypes.BFloat16 {
		b, err := gpuConvertSameShape(rhs, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		r = b
	...
	case backends.OpTypeLogicalAnd, backends.OpTypeLogicalOr, backends.OpTypeLogicalXor:
		...
		dt = metalDTypeBoolPred()
	default:
		if workDT, code, ok := elementwiseIntegerWorkDType(l.shape.DType); ok {
			if l.shape.DType != workDT {
				lhsW, err := gpuConvertSameShape(l, workDT)
				if err != nil {
					return nil, err
				}
				rhsW, err := gpuConvertSameShape(r, workDT)
				if err != nil {
					return nil, err
				}
				l, r = lhsW, rhsW
	...
	var dst *Buffer
	if node.shape.DType == dtypes.Bool {
		dst = allocDuringExec(node.shape)
	} else if outIsBF16 {
		dst = allocDuringExec(shapes.Make(dtypes.Float32, node.shape.Dimensions...))
	} else {
		dst = allocDuringExec(shapes.Make(l.shape.DType, node.shape.Dimensions...))
	}
	...
	if ret := C.metal_binary_op(cName, l.mtl, r.mtl, dst.mtl, n, dt); ret != 0 {
	...
	if node.shape.DType != dtypes.Bool {
		if workDT, _, ok := elementwiseIntegerWorkDType(origElemDT); ok && workDT != node.shape.DType {
			return gpuConvertSameShape(dst, node.shape.DType)
		}
	}
	return dst, nil
...
func executeReduce(node *Node, input *Buffer) (*Buffer, error) {
	...
	inShape := input.shape
	origReduceDT := inShape.DType
	var dt C.int

	workInput := input
	workDType := inShape.DType
	needsBF16Out := false
	needsIntNarrowOut := false
	switch node.opType {
	case backends.OpTypeReduceSum, backends.OpTypeReduceProduct, backends.OpTypeReduceMax, backends.OpTypeReduceMin:
		if inShape.DType == dtypes.BFloat16 {
			b, err := gpuConvertSameShape(input, dtypes.Float32)
			if err != nil {
				return nil, err
			}
			workInput = b
			workDType = dtypes.Float32
			needsBF16Out = true
		} else if w, _, ok := elementwiseIntegerWorkDType(workDType); ok && w != workDType {
			b, err := gpuConvertSameShape(workInput, w)
			if err != nil {
				return nil, err
			}
			workInput = b
			workDType = w
			needsIntNarrowOut = origReduceDT != w
		}
	}
	...
	case backends.OpTypeReduceSum, backends.OpTypeReduceProduct, backends.OpTypeReduceMax, backends.OpTypeReduceMin:
		...
		case dtypes.Uint64:
			dt = 7
		default:
			dt = dtypeToMetal(workDType)
		}
		if dt < 0 {
			return nil, errors.Errorf("metal reduce: unsupported dtype %s", workDType)
		}
	default:
		dt = dtypeToMetal(workDType)
		if dt < 0 {
	...
	for _, axis := range axes {
		...
		if axis != rank-1 {
			...
			transposed := allocDuringExec(shapes.Make(workDType, transposedDims...))
			if err := transposeBufferUnified(current, transposed, currentDims, transposedDims, perm, workDType); err != nil {
				return nil, err
			}
			current = transposed
		...
		dst := allocDuringExec(shapes.Make(workDType, newDims...))
	...
	if needsBF16Out {
		return gpuConvertSameShape(current, dtypes.BFloat16)
	}
	if needsIntNarrowOut {
		return gpuConvertSameShape(current, origReduceDT)
	}
	return current, nil
}

// transposeBufferCPU performs a generic transpose on CPU via shared memory.
func transposeBufferCPU(src, dst *Buffer, srcDims, dstDims []int, perm []int, elemSize int) error {
	if err := gpuSyncBeforeHostRead(); err != nil {
		return err
	}
	size := 1
...
func transposeBufferUnified(src, dst *Buffer, srcDims, dstDims []int, perm []int, dt dtypes.DType) error {
	...
	if rank != len(perm) || rank != len(dstDims) || rank > maxTransposeRank {
		return transposeBufferCPU(src, dst, srcDims, dstDims, perm, int(dt.Size()))
	}
...
func whereBuffers(outShape shapes.Shape, pred, onTrue, onFalse *Buffer) (*Buffer, error) {
	if outShape.DType == dtypes.BFloat16 {
		f32Shape := shapes.Make(dtypes.Float32, outShape.Dimensions...)
		t32, err := gpuConvertSameShape(onTrue, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		f32, err := gpuConvertSameShape(onFalse, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		dstF, err := whereBuffers(f32Shape, pred, t32, f32)
		if err != nil {
			return nil, err
		}
		return gpuConvertSameShape(dstF, dtypes.BFloat16)
	}
...
func metalSortLoadDirectPairBytes(flat, lhs, rhs *Buffer, baseElem, axisStride, es, ii, jj uint32) error {
	if ret := C.metal_sort_load_pair_direct_bytes(flat.mtl, lhs.mtl, rhs.mtl,
		C.uint32_t(baseElem), C.uint32_t(axisStride), C.uint32_t(es),
		C.uint32_t(ii), C.uint32_t(jj)); ret != 0 {
		return errors.Errorf("metal_sort_load_pair_direct_bytes failed: %d", ret)
	}
	return nil
}
...
func executeDynamicSlice(node *Node, inputs []*Buffer) (*Buffer, error) {
	data := node.data.(*dynamicSliceData)
	input := inputs[0]
	opShape := input.shape
	rank := opShape.Rank()
	outDims := data.sliceSizes

	if rank == 0 {
		dst := allocDuringExec(node.shape)
		nBytes := int(opShape.DType.Size()) * node.shape.Size()
		if nBytes > 0 {
			if err := gpuSyncBeforeHostRead(); err != nil {
				return nil, err
			}
			C.memcpy(dst.contents(), input.contents(), C.size_t(nBytes))
		}
		return dst, nil
	}
	...
	starts := make([]uint32, rank)
	if data.packedStartIndices {
		buf := inputs[1]
		for i := range rank {
			s, err := readDynamicUpdateStartIndex(buf, i)
			if err != nil {
				return nil, err
			}
			starts[i] = clampDynamicSliceStart(opShape.Dimensions[i], outDims[i], s)
		}
	} else {
		if len(inputs) != 1+rank {
			return nil, errors.Errorf("dynamic slice: want %d start buffers, got %d", rank, len(inputs)-1)
		}
		for i := range rank {
			s, err := readDynamicUpdateStartIndex(inputs[1+i], 0)
			if err != nil {
				return nil, err
			}
			starts[i] = clampDynamicSliceStart(opShape.Dimensions[i], outDims[i], s)
	...
	inStrides := computeStrides(opShape.Dimensions)
	config := make([]uint32, 1+4*rank)
	config[0] = uint32(rank)
	for i := 0; i < rank; i++ {
		config[1+i] = starts[i]
	...
	dst := allocDuringExec(node.shape)
	total := node.shape.Size()
	es := int(node.shape.DType.Size())

	if err := cDynamicSlice(input, dst, configBuf, total, len(config), es); err != nil {
		return nil, err
	}
	return dst, nil
}

func readDynamicUpdateStartIndex(buf *Buffer, linearElem int) (int64, error) {
	dt := buf.shape.DType
	es := int(dt.Size())
	if linearElem < 0 || linearElem >= buf.shape.Size() {
		return 0, errors.Errorf("start index elem %d out of range for shape %s", linearElem, buf.shape)
	}
	p := unsafe.Add(buf.contents(), linearElem*es)
	switch dt {
	case dtypes.Int8:
	...
	case dtypes.Uint64:
		return int64(*(*uint64)(p)), nil
	default:
		return 0, errors.Errorf("dynamic update slice: index dtype %s not supported", dt)
	}
}

func clampDynamicUpdateStart(opDim, upDim int, s int64) uint32 {
	maxStart := opDim - upDim
	if maxStart < 0 {
		maxStart = 0
	}
	if s < 0 {
	...
	if s > int64(maxStart) {
		return uint32(maxStart)
	}
	return uint32(s)
}

func clampDynamicSliceStart(opDim, sliceDim int, s int64) uint32 {
	maxStart := opDim - sliceDim
	if maxStart < 0 {
		maxStart = 0
	}
	if s < 0 {
...
func executeDynamicUpdateSlice(node *Node, inputs []*Buffer) (*Buffer, error) {
	data := node.data.(*dynamicUpdateSliceData)
	operand := inputs[0]
	update := inputs[1]
	opShape := operand.shape
	rank := opShape.Rank()
	if rank == 0 {
		dst := allocDuringExec(opShape)
		nBytes := int(opShape.DType.Size()) * opShape.Size()
		if nBytes > 0 {
			if err := gpuSyncBeforeHostRead(); err != nil {
				return nil, err
			}
			C.memcpy(dst.contents(), update.contents(), C.size_t(nBytes))
		}
		return dst, nil
	}
	...
	if data.packedStartIndices {
		...
		for i := range rank {
			...
			starts[i] = clampDynamicUpdateStart(opShape.Dimensions[i], update.shape.Dimensions[i], s)
		}
	} else {
		if len(inputs) != 2+rank {
			return nil, errors.Errorf("dynamic update slice: want %d start buffers, got %d", rank, len(inputs)-2)
		}
		for i := range rank {
			s, err := readDynamicUpdateStartIndex(inputs[2+i], 0)
			if err != nil {
				return nil, err
			}
			starts[i] = clampDynamicUpdateStart(opShape.Dimensions[i], update.shape.Dimensions[i], s)
	...
	upStrides := computeStrides(update.shape.Dimensions)
	config := make([]uint32, 1+5*rank)
	config[0] = uint32(rank)
	copy(config[1:1+rank], starts)
	for i := range rank {
		config[1+rank+i] = uint32(opShape.Dimensions[i])
		config[1+2*rank+i] = uint32(update.shape.Dimensions[i])
		config[1+3*rank+i] = uint32(opStrides[i])
		config[1+4*rank+i] = uint32(upStrides[i])
	}
	...
	dst := allocDuringExec(opShape)
	total := opShape.Size()
	es := int(opShape.DType.Size())

	if err := cDynamicUpdateSlice(operand, update, dst, configBuf, total, len(config), es); err != nil {
		return nil, err
	}
...
func executeIota(node *Node) (*Buffer, error) {
	...
	if outShape.DType == dtypes.BFloat16 {
		f32Shape := shapes.Make(dtypes.Float32, outShape.Dimensions...)
		dt := dtypeToMetalIota(dtypes.Float32)
		dstF := allocDuringExec(f32Shape)
		if ret := C.metal_iota(dstF.mtl, C.uint32_t(batchSize), C.uint32_t(iotaSize),
...
func executeScatter(node *Node, operand, indices, updates *Buffer) (*Buffer, error) {
	...
	// float16 / bfloat16: no 16-bit device atomics in MSL — accumulate in float32, convert back.
	if operand.shape.DType == dtypes.Float16 || operand.shape.DType == dtypes.BFloat16 {
		...
		return gpuConvertSameShape(dstF32, operand.shape.DType)
	}

	sk := scatterElemKind(operand.shape.DType)
	if sk < 0 {
		return nil, errors.Errorf(
			"scatter: metal scatter supports float16/bfloat16/float32/int32/uint32/int64/uint64 only, got operand %s (updates %s)",
			operand.shape.DType, updates.shape.DType)
	}
...
func executeArgMinMax(node *Node, input *Buffer) (*Buffer, error) {
	if input.shape.DType != dtypes.Float32 && input.shape.DType != dtypes.Float16 && input.shape.DType != dtypes.BFloat16 {
		return nil, errors.Errorf("argminmax: metal supports bfloat16/float16/f32 input only, got %s", input.shape.DType)
	}
	...
	if outDType == dtypes.Int64 {
		...
		if ret := C.metal_argminmax(in.mtl, tmp.mtl,
	...
	dst := allocDuringExec(node.shape)
	if ret := C.metal_argminmax(in.mtl, dst.mtl,
		C.uint32_t(prefixSize), C.uint32_t(reduceSize), C.uint32_t(suffixSize),
		isMin, dt); ret != 0 {
		return nil, errors.Errorf("metal_argminmax failed: %d", ret)
	}
...
func executeFusedQuantizedDense(node *Node, inputs []*Buffer) (*Buffer, error) {
	...
	xEffective := xBuf
	if xBuf.shape.DType == dtypes.Float16 || xBuf.shape.DType == dtypes.BFloat16 {
		var err error
		...
		return nil, errors.Errorf("FusedQuantizedDense: x must be float16, bfloat16, or float32, got %s", xBuf.shape.DType)
	}
	...
	outKernel := out
	if node.shape.DType == dtypes.Float16 || node.shape.DType == dtypes.BFloat16 {
	...
	if node.shape.DType == dtypes.Float16 || node.shape.DType == dtypes.BFloat16 {
		...
		srcDt := C.int(1)
		dstDt := C.int(0)
		if node.shape.DType == dtypes.BFloat16 {
			dstDt = 12
		}
		if ret := C.metal_convert_dtype(outKernel.mtl, out.mtl, n, srcDt, dstDt); ret != 0 {
			return nil, errors.Errorf("metal_quantized_dense f32->low precision: %d", ret)
		}
	}
	return out, nil
}
...
func executeConvGeneral(node *Node, input, kernel *Buffer) (*Buffer, error) {
	...
	if dt != dtypes.Float32 && dt != dtypes.Float16 && dt != dtypes.BFloat16 {
		return nil, errors.Errorf("conv_general: metal supports float16/float32/bfloat16, got input %s", dt)
	}
	if kernel.shape.DType != dt {
		return nil, errors.Errorf("conv_general: kernel dtype %s must match input %s", kernel.shape.DType, dt)
	}

	workIn, workK := input, kernel
	if dt == dtypes.BFloat16 {
		var err error
		workIn, err = gpuConvertSameShape(input, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		workK, err = gpuConvertSameShape(kernel, dtypes.Float32)
		if err != nil {
			return nil, err
		}
	}

	kernelOutDT := node.shape.DType
	if kernelOutDT == dtypes.BFloat16 {
		kernelOutDT = dtypes.Float32
	}
	if dtypeToMetal(kernelOutDT) < 0 {
		return nil, errors.Errorf("conv_general: unsupported dtype %s", kernelOutDT)
	}
	...
	dst := allocDuringExec(shapes.Make(kernelOutDT, outShape.Dimensions...))
	total := C.uint32_t(outShape.Size())
	mdt := dtypeToMetal(kernelOutDT)

	if ret := C.metal_conv_general(workIn.mtl, workK.mtl, dst.mtl, configBuf.mtl,
...
func executeReduceWindow(node *Node, input *Buffer) (*Buffer, error) {
	...
	if idt != dtypes.Float32 && idt != dtypes.Float16 && idt != dtypes.BFloat16 {
		return nil, errors.Errorf("reduce_window: metal supports float16/float32/bfloat16, got %s", idt)
	}

	workIn := input
	kernelElemDT := idt
	if idt == dtypes.BFloat16 {
		b, err := gpuConvertSameShape(input, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		workIn = b
		kernelElemDT = dtypes.Float32
	}
	if dtypeToMetal(kernelElemDT) < 0 {
		return nil, errors.Errorf("reduce_window: unsupported dtype %s", kernelElemDT)
	}
	...
	dst := allocDuringExec(shapes.Make(kernelElemDT, outShape.Dimensions...))
	total := C.uint32_t(outShape.Size())
	dt := dtypeToMetal(kernelElemDT)

	if ret := C.metal_reduce_window(workIn.mtl, dst.mtl, configBuf.mtl,
...
func executeSelectAndScatterMax(node *Node, inputs []*Buffer) (*Buffer, error) {
	operand := inputs[0]
	dy := inputs[1]
	idt := operand.shape.DType
	if idt != dtypes.Float32 && idt != dtypes.Float16 && idt != dtypes.BFloat16 {
		return nil, errors.Errorf("select_and_scatter_max: metal supports float16/float32/bfloat16, got %s", idt)
	}

	data := node.data.(*selectAndScatterMaxData)
	rank := operand.shape.Rank()
	if rank != 4 {
		return nil, errors.Errorf("select_and_scatter_max: want rank 4, got %d", rank)
	}

	exp, err := shapeinference.ReduceWindowOp(
		operand.shape, data.windowDimensions, data.strides,
		data.baseDilations, data.windowDilations, data.paddings)
	if err != nil {
		return nil, errors.WithMessage(err, "select_and_scatter_max")
	}
	if !dy.shape.Equal(exp) {
		return nil, errors.Errorf(
			"select_and_scatter_max: dy shape %s, expected %s", dy.shape, exp)
	}

	opF := operand
	dyF := dy
	if idt == dtypes.Float16 || idt == dtypes.BFloat16 {
		var convErr error
		opF, convErr = gpuConvertSameShape(operand, dtypes.Float32)
		if convErr != nil {
			return nil, convErr
		}
		dyF, convErr = gpuConvertSameShape(dy, dtypes.Float32)
		if convErr != nil {
			return nil, convErr
		}
	}

	inStrides := computeStrides(opF.shape.Dimensions)
	configLen := 1 + 9*rank
	config := make([]uint32, configLen)
	config[0] = uint32(rank)
	off := 1
	for i := 0; i < rank; i++ {
		config[off+i] = uint32(opF.shape.Dimensions[i])
	}
	off += rank
	for i := 0; i < rank; i++ {
		config[off+i] = uint32(dyF.shape.Dimensions[i])
	}
	off += rank
	for i := 0; i < rank; i++ {
		config[off+i] = uint32(data.windowDimensions[i])
	...
	dstF := allocDuringExec(shapes.Make(dtypes.Float32, opF.shape.Dimensions...))
	nBytesF := dstF.shape.Size() * int(dtypes.Float32.Size())
	if nBytesF > 0 {
		C.memset(dstF.contents(), 0, C.size_t(nBytesF))
	}

	totalOut := dyF.shape.Size()
	if err := cMaxPool2DGradNCHW(opF, dyF, dstF, configBuf, totalOut, 1); err != nil {
		return nil, err
	}

	if idt == dtypes.Float16 {
		return gpuConvertSameShape(dstF, dtypes.Float16)
	}
	if idt == dtypes.BFloat16 {
		return gpuConvertSameShape(dstF, dtypes.BFloat16)
	}
...
// isSimpleMatMul is the rank-2 non-batched case [M,K]×[K,N]: lhs contracts axis 1, rhs axis 0.
func isSimpleMatMul(
	lhsShape shapes.Shape, lhsContracting, lhsBatch []int,
	rhsShape shapes.Shape, rhsContracting, rhsBatch []int,
) bool {
	if len(lhsBatch) != 0 || len(rhsBatch) != 0 {
		return false
	}
	if lhsShape.Rank() != 2 || rhsShape.Rank() != 2 {
		return false
	}
	if len(lhsContracting) != 1 || len(rhsContracting) != 1 {
		return false
	}
	return lhsContracting[0] == 1 && rhsContracting[0] == 0
}
...
func executeDot(node *Node, lhs, rhs *Buffer) (*Buffer, error) {
	...
	kernelOutDT := outDT
	if outDT == dtypes.BFloat16 {
		kernelOutDT = dtypes.Float32
	}
	dst := allocDuringExec(shapes.Make(kernelOutDT, node.shape.Dimensions...))
	dt := dtypeToMetal(kernelOutDT)
	if ret := C.metal_dot_general(lhsK.mtl, rhsK.mtl, dst.mtl,
		1, C.uint32_t(m), C.uint32_t(k), C.uint32_t(n), dt); ret != 0 {
		return nil, errors.Errorf("metal_dot_general failed: %d", ret)
	}
	if kernelOutDT != outDT {
		return gpuConvertSameShape(dst, outDT)
	}
	return dst, nil
}
...
func executeDotGeneral(node *Node, lhs, rhs *Buffer) (*Buffer, error) {
	...
	// Fast path only for true [M,K]×[K,N] matmul; other rank-2 DotGeneral
	// patterns (e.g. contracting axis 0 on both sides) must use the general path.
	if isSimpleMatMul(lShape, data.lhsContractingAxes, data.lhsBatchAxes,
		rShape, data.rhsContractingAxes, data.rhsBatchAxes) {
	...
	kernelOutDT := node.shape.DType
	dstShape := node.shape
	if lShape.DType == dtypes.BFloat16 {
		var err error
		lhsK, err = gpuConvertSameShape(lhsTrans, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		rhsK, err = gpuConvertSameShape(rhsTrans, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		kernelOutDT = dtypes.Float32
		dstShape = shapes.Make(dtypes.Float32, node.shape.Dimensions...)
	}

	dst := allocDuringExec(dstShape)
	dt := dtypeToMetal(kernelOutDT)
	if ret := C.metal_dot_general(lhsK.mtl, rhsK.mtl, dst.mtl,
		C.uint32_t(batch), C.uint32_t(m), C.uint32_t(k), C.uint32_t(n), dt); ret != 0 {
		return nil, errors.Errorf("metal_dot_general failed: %d", ret)
	}
	if kernelOutDT != node.shape.DType {
		return gpuConvertSameShape(dst, node.shape.DType)
	}
	return dst, nil
}

// executeFusedDense runs the fused matmul → bias → activation kernel (see kernels/fused_ops.metal).
func executeFusedDense(node *Node, inputs []*Buffer) (*Buffer, error) {
	data := node.data.(*fusedDenseData)
	x, w := inputs[0], inputs[1]
	hasBias := C.uint32_t(0)
	var bias *Buffer
	if len(inputs) > 2 {
		hasBias = 1
		bias = inputs[2]
	...
	rank := x.shape.Rank()
	k := x.shape.Dimensions[rank-1]
	m := x.shape.Size() / k
	n := w.shape.Dimensions[1]
	...
	if outDT == dtypes.BFloat16 {
		kernelOutDT = dtypes.Float32
		var err error
		xK, err = gpuConvertSameShape(x, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		wK, err = gpuConvertSameShape(w, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		if bias != nil {
			biasK, err = gpuConvertSameShape(bias, dtypes.Float32)
			if err != nil {
				return nil, err
			}
		}
	...
	dt := dtypeToMetal(kernelOutDT)
	if dt < 0 {
		return nil, errors.Errorf("fused dense: unsupported kernel dtype %s", kernelOutDT)
	}
	...
	act := C.uint32_t(uint32(data.activation))

	if ret := C.metal_fused_dense(xK.mtl, wK.mtl, biasM, dst.mtl,
		C.uint32_t(m), C.uint32_t(k), C.uint32_t(n),
		hasBias, act, ge, dt); ret != 0 {
		return nil, errors.Errorf("metal_fused_dense failed: %d", ret)
	}
	if kernelOutDT != outDT {
		return gpuConvertSameShape(dst, outDT)
	}
...
// executeFusedAttentionQKVProjection fuses the combined projection into Q/K/V device buffers.
func executeFusedAttentionQKVProjection(node *Node, inputs []*Buffer) ([]*Buffer, error) {
	data := node.data.(*fusedQKVData)
	x, w := inputs[0], inputs[1]
	idx := 2
	var bq, bk, bv *Buffer
	if data.HasBiasQ {
	...
	rank := x.shape.Rank()
	k := w.shape.Dimensions[0]
	m := x.shape.Size() / x.shape.Dimensions[rank-1]
	qd := uint32(data.QueryDim)
	kvd := uint32(data.KeyValueDim)

	outDT := node.multiOutputsShapes[0].DType
	kernelOutDT := outDT
	xK, wK := x, w
	promoteBF16 := outDT == dtypes.BFloat16
	if promoteBF16 {
		kernelOutDT = dtypes.Float32
		var err error
		xK, err = gpuConvertSameShape(x, dtypes.Float32)
		...
		if bq != nil {
			bq, err = gpuConvertSameShape(bq, dtypes.Float32)
			if err != nil {
				return nil, err
			}
		}
		if bk != nil {
			bk, err = gpuConvertSameShape(bk, dtypes.Float32)
			if err != nil {
				return nil, err
			}
		}
		if bv != nil {
			bv, err = gpuConvertSameShape(bv, dtypes.Float32)
			if err != nil {
				return nil, err
			}
		}
	...
	qDst := allocDuringExec(shapes.Make(kernelOutDT, node.multiOutputsShapes[0].Dimensions...))
	kDst := allocDuringExec(shapes.Make(kernelOutDT, node.multiOutputsShapes[1].Dimensions...))
	vDst := allocDuringExec(shapes.Make(kernelOutDT, node.multiOutputsShapes[2].Dimensions...))
	...
	dt := dtypeToMetal(kernelOutDT)
	if dt < 0 {
		return nil, errors.Errorf("fused qkv: unsupported kernel dtype %s", kernelOutDT)
	}

	if ret := C.metal_fused_qkv_projection(xK.mtl, wK.mtl, bqM, bkM, bvM,
		qDst.mtl, kDst.mtl, vDst.mtl,
		C.uint32_t(m), C.uint32_t(k), C.uint32_t(qd), C.uint32_t(kvd),
		hq, hk, hv, dt); ret != 0 {
		return nil, errors.Errorf("metal_fused_qkv_projection failed: %d", ret)
	}

	if promoteBF16 {
		qo, err := gpuConvertSameShape(qDst, outDT)
		if err != nil {
			return nil, err
		}
		ko, err := gpuConvertSameShape(kDst, outDT)
		if err != nil {
			return nil, err
		}
		vo, err := gpuConvertSameShape(vDst, outDT)
		if err != nil {
			return nil, err
		}
		return []*Buffer{qo, ko, vo}, nil
	}
	return []*Buffer{qDst, kDst, vDst}, nil
}
...
func executeFusedSoftmax(node *Node, input *Buffer) (*Buffer, error) {
	...
	dst := allocDuringExec(execShape)
	dt := dtypeToMetal(in.shape.DType)
	if dt < 0 {
		return nil, errors.Errorf("metal_fused_softmax: unsupported dtype %s", in.shape.DType)
	}

	if ret := C.metal_fused_softmax(in.mtl, dst.mtl,
...
func executeFusedGelu(node *Node, input *Buffer) (*Buffer, error) {
	...
	dt := dtypeToMetal(in.shape.DType)
	if dt < 0 {
		return nil, errors.Errorf("metal_fused_gelu: unsupported dtype %s", in.shape.DType)
	}

	if ret := C.metal_fused_gelu(in.mtl, dst.mtl,
		C.uint32_t(execShape.Size()), exact, dt); ret != 0 {
...
func executeFusedLayerNorm(node *Node, inputs []*Buffer) (*Buffer, error) {
	...
	var promotedGamma, promotedBeta *Buffer
	defer func() {
		runtime.KeepAlive(promotedGamma)
		runtime.KeepAlive(promotedBeta)
	}()

	outIsBF16 := node.shape.DType == dtypes.BFloat16
	execShape := node.shape
	xIn := x
	gammaM := C.MetalBuffer(nil)
	betaM := C.MetalBuffer(nil)
	hasGamma := C.int(0)
	hasBeta := C.int(0)
	if len(inputs) > 1 {
		hasGamma = 1
	...
	if x.shape.DType == dtypes.BFloat16 {
		b, err := gpuConvertSameShape(x, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		xIn = b
		...
		if len(inputs) > 1 {
			var err error
			promotedGamma, err = gpuConvertSameShape(inputs[1], dtypes.Float32)
			if err != nil {
				return nil, err
			}
			gammaM = promotedGamma.mtl
		}
		if len(inputs) > 2 {
			var err error
			promotedBeta, err = gpuConvertSameShape(inputs[2], dtypes.Float32)
			if err != nil {
				return nil, err
			}
			betaM = promotedBeta.mtl
		}
	} else {
		if len(inputs) > 1 {
			gammaM = inputs[1].mtl
	...
	dst := allocDuringExec(execShape)
	dt := dtypeToMetal(xIn.shape.DType)
	if dt < 0 {
		return nil, errors.Errorf("metal_fused_layernorm: unsupported dtype %s", xIn.shape.DType)
	}

	if ret := C.metal_fused_layernorm(xIn.mtl, gammaM, betaM, dst.mtl,
...
func sdpaMaterializeBoolMask(mask *Buffer, qDtype dtypes.DType) (*Buffer, error) {
	if qDtype == dtypes.BFloat16 {
		f32Mask, err := sdpaMaterializeBoolMask(mask, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		return gpuConvertSameShape(f32Mask, dtypes.BFloat16)
	}
...
func executeFusedSDPA(node *Node, inputs []*Buffer) (*Buffer, error) {
	...
	qShape := q.shape
	outBF16 := qShape.DType == dtypes.BFloat16
	qK, kK, vK := q, k, v
	dst := allocDuringExec(node.shape)
	if outBF16 {
		var err error
		qK, err = gpuConvertSameShape(q, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		kK, err = gpuConvertSameShape(k, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		vK, err = gpuConvertSameShape(v, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		dst = allocDuringExec(shapes.Make(dtypes.Float32, node.shape.Dimensions...))
	...
	sdpaElemDType := qShape.DType
	if outBF16 {
		sdpaElemDType = dtypes.Float32
	}
	if len(inputs) > 3 {
		mask = inputs[3]
		...
		if mask.shape.DType == dtypes.Bool {
			mat, err := sdpaMaterializeBoolMask(mask, sdpaElemDType)
			if err != nil {
				return nil, err
			}
			mask = mat
			...
			if outBF16 {
				mf, err := gpuConvertSameShape(mask, dtypes.Float32)
				if err != nil {
					return nil, err
				}
				mask = mf
	...
	if err := cFusedSDPA(qK, kK, vK, dst, mask, batch, numHeads, uint32(data.numKVHeads), seqLen, kvLen, headDim,
		data.scale, data.causal, maskType, maskBatchStride, maskHeadStride, sdpaElemDType); err != nil {
		return nil, err
	}
	if outBF16 {
		return gpuConvertSameShape(dst, dtypes.BFloat16)
...
// 0=f16, 1=f32, 3=i32, 4=i64, 5=u32, 6=u64, 7=bool, 8=i8, 9=i16, 10=u8, 11=u16, 12=bfloat16.
...
func executeBatchNormTrainingGPU(node *Node, inputs []*Buffer) ([]*Buffer, error) {
	...
	promoteBF16 := x.shape.DType == dtypes.BFloat16
	if promoteBF16 {
		var err error
		x, err = gpuConvertSameShape(x, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		scale, err = gpuConvertSameShape(scale, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		offset, err = gpuConvertSameShape(offset, dtypes.Float32)
		if err != nil {
			return nil, err
		}
	}
	...
	var outNorm, outMean, outVar *Buffer
	if promoteBF16 {
		outNorm = allocDuringExec(shapes.Make(dtypes.Float32, node.multiOutputsShapes[0].Dimensions...))
		outMean = allocDuringExec(shapes.Make(dtypes.Float32, node.multiOutputsShapes[1].Dimensions...))
		outVar = allocDuringExec(shapes.Make(dtypes.Float32, node.multiOutputsShapes[2].Dimensions...))
	} else {
	...
	n0, err := gpuConvertSameShape(outNorm, dtypes.BFloat16)
	if err != nil {
		return nil, err
	}
	n1, err := gpuConvertSameShape(outMean, dtypes.BFloat16)
	if err != nil {
		return nil, err
	}
	n2, err := gpuConvertSameShape(outVar, dtypes.BFloat16)
	if err != nil {
		return nil, err
	}
	return []*Buffer{n0, n1, n2}, nil
}
...
func executeBatchNormGradientGPU(node *Node, inputs []*Buffer) ([]*Buffer, error) {
	...
	if promoteBF16 {
		...
		mean, err = gpuConvertSameShape(mean, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		variance, err = gpuConvertSameShape(variance, dtypes.Float32)
		if err != nil {
			return nil, err
		}
		dy, err = gpuConvertSameShape(dy, dtypes.Float32)
		if err != nil {
			return nil, err
		}
	}
	...
	var dx, dgamma, dbeta *Buffer
	if promoteBF16 {
		dx = allocDuringExec(shapes.Make(dtypes.Float32, node.multiOutputsShapes[0].Dimensions...))
		dgamma = allocDuringExec(shapes.Make(dtypes.Float32, node.multiOutputsShapes[1].Dimensions...))
		dbeta = allocDuringExec(shapes.Make(dtypes.Float32, node.multiOutputsShapes[2].Dimensions...))
	} else {
	...
	o0, err := gpuConvertSameShape(dx, dtypes.BFloat16)
	if err != nil {
		return nil, err
	}
	o1, err := gpuConvertSameShape(dgamma, dtypes.BFloat16)
	if err != nil {
		return nil, err
	}
	o2, err := gpuConvertSameShape(dbeta, dtypes.BFloat16)
	if err != nil {
		return nil, err
	}
	return []*Buffer{o0, o1, o2}, nil
}
...
func flatFromBuffer(buf *Buffer) any {
	...
	case dtypes.BFloat16:
		return unsafe.Slice((*bfloat16.BFloat16)(ptr), size)
	case dtypes.Int32:
		return unsafe.Slice((*int32)(ptr), size)
	case dtypes.Int16:
		return unsafe.Slice((*int16)(ptr), size)
	case dtypes.Int64:
	...
	case dtypes.Uint16:
		return unsafe.Slice((*uint16)(ptr), size)
	case dtypes.Uint32:
		return unsafe.Slice((*uint32)(ptr), size)
	case dtypes.Uint64:
